...
from FEDformer.layers.Autoformer_EncDec import Encoder, Decoder, EncoderLayer, DecoderLayer, my_Layernorm, series_decomp, \
    series_decomp_multi
import math
import numpy as np
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        enc_out, attns, latent_enc = self.encoder(enc_out, attn_mask=enc_self_mask)
        # dec
        dec_out = self.dec_embedding(seasonal_init, x_mark_dec)
        seasonal_part, trend_part, latent_dec = self.decoder(dec_out, enc_out, x_mask=dec_self_mask,
                                                             cross_mask=dec_enc_mask,
                                                             trend=trend_init)
        # final
        dec_out = trend_part + seasonal_part
        latent_reps = latent_enc + latent_dec

        if self.output_attention:
            return dec_out[:, -self.pred_len:, :], attns, latent_reps
        else:
            return dec_out[:, -self.pred_len:, :], latent_reps  # [B, L, D]
